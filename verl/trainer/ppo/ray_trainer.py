...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        self.train_dataset = RLHFDataset(parquet_files=self.config.data.train_files,
                                         ...
                                         prompt_is_text=self.config.data.get('prompt_is_text', False),
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            reward_tensor = self.val_reward_fn(test_batch, step=self.global_steps)
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.use_rm:
                            ...
                            use_actor_as_rm = self.config.reward_model.get('use_actor_as_rm', False)
                            if use_actor_as_rm:
                                reward_tensor = self.actor_rollout_wg.compute_actor_rm_score(batch)
                            else:
                                reward_tensor = self.rm_wg.compute_rm_score(batch)
                            batch = batch.union(reward_tensor)

                        # we combine with rule-based rm
                        reward_tensor = self.reward_fn(batch, step=self.global_steps)
